...
test('getDatabaseName - string', () => {
  const name = getDatabaseName({ options: { database: 'maeum-string' } });
  expect(name).toEqual('maeum-string');
});

test('getDatabaseName - uint8array', () => {
  const name = getDatabaseName({ options: { database: Buffer.from('maeum-uint8array') } });
  expect(name).toEqual('maeum-uint8array');
});

test('getDatabaseName - undefined', () => {
  const name = getDatabaseName({ options: { database: undefined } });
...
test('getRelationHash - one-to-one', () => {
  const hash = getRelationHash({ entityName: 'a', inverseEntityName: 'b', relationType: 'one-to-one' });
  expect('YTpiOm9uZS10by1vbmU=').toEqual(hash);
});

test('getRelationHash - one-to-many', () => {
  const hash = getRelationHash({ entityName: 'a', inverseEntityName: 'b', relationType: 'one-to-many' });
  expect('YTpiOm9uZS10by1tYW55').toEqual(hash);
});

test('getRelationHash - many-to-one', () => {
  const hash = getRelationHash({ entityName: 'a', inverseEntityName: 'b', relationType: 'many-to-one' });
  expect('YTpiOm9uZS10by1tYW55').toEqual(hash);
});

test('getRelationHash - many-to-many', () => {
  const hash = getRelationHash({ entityName: 'a', inverseEntityName: 'b', relationType: 'many-to-many' });
